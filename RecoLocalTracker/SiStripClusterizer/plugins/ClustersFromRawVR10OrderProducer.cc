  ...
  for (auto const conn : clusterizer.currentConnection()) {
	...
	while (unpacker.hasData()) {
	  ...
	  unpacker++;
        ...
        // re-order digits from physical to readout order
	std::vector<int16_t> digisOrd;
        if ( !digis.empty() ) { 
          uint16_t physical;
          uint16_t readout; 
          for ( uint16_t i = 0, n = digis.size(); i < n; i++ ) {
            physical = i%128;
            readout = ( 4*((static_cast<uint16_t>((static_cast<float>(physical)/8.0)))%4) + static_cast<uint16_t>(static_cast<float>(physical)/32.0) + 16*(physical%8) );
            (i/128) ? readout=readout*2+1 : readout=readout*2; // un-multiplex data
            digisOrd.push_back( digis[readout] );
          }
        }
	...
	rawAlgos.SuppressVirginRawData(id, firstAPV, digisOrd, zsdigis);  
