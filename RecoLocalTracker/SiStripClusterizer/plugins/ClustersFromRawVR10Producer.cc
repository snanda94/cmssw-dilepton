...
namespace {
  sistrip::FEDBuffer*fillBufferVR10(int fedId, const FEDRawDataCollection& rawColl) {
  ...
  class ClusterFillerVR10 final : public StripClusterizerAlgorithm::output_t::Getter {
  public:
    ClusterFillerVR10(const FEDRawDataCollection& irawColl,
...
class SiStripClusterizerFromRawVR10 final : public edm::stream::EDProducer<>  {
  
 public:
  
  explicit SiStripClusterizerFromRawVR10(const edm::ParameterSet& conf) :
  ...
  void produce(edm::Event& ev, const edm::EventSetup& es) {
    ...
    std::auto_ptr< edmNew::DetSetVector<SiStripCluster> > 
      output( onDemand ?
	      new edmNew::DetSetVector<SiStripCluster>(std::shared_ptr<edmNew::DetSetVector<SiStripCluster>::Getter>(std::make_shared<ClusterFillerVR10>(*rawData, *clusterizer_, 
...
DEFINE_FWK_MODULE(SiStripClusterizerFromRawVR10);




void SiStripClusterizerFromRawVR10::initialize(const edm::EventSetup& es) {
...
void SiStripClusterizerFromRawVR10::run(const FEDRawDataCollection& rawColl,
				     edmNew::DetSetVector<SiStripCluster> & output) {
  
  ClusterFillerVR10 filler(rawColl, *clusterizer_, *rawAlgos_, doAPVEmulatorCheck_);
...
void ClusterFillerVR10::fill(StripClusterizerAlgorithm::output_t::FastFiller & record) {
try {
  incReady();
  ...
  for (auto const conn : clusterizer.currentConnection()) {
    ...
    if (!done[fedId]) { buffers[fedId].reset(fillBufferVR10(fedId, rawColl)); done[fedId]=true;}
