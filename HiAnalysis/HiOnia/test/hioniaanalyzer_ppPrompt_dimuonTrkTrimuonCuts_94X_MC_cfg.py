...
muonSelection  = "GlbOrTrk" # Single muon selection: Glb(isGlobal), GlbTrk(isGlobal&&isTracker), Trk(isTracker), TwoGlbAmongThree (which requires two isGlobal for a trimuon, and one isGlobal for a dimuon) are available
...
options.outputFile = "Oniatree_MC_dimuonTrkTrimuonCuts.root"
...
process.onia2MuMuPatGlbGlb.DimuTrkSelection      = cms.string("3.3 < mass && mass < 7.3 && abs(daughter('muon1').innerTrack.dz - daughter('track').track.dz) < 0.7 && abs(daughter('track').eta()) < 2.4 && ( (abs(daughter('track').eta()) < 1. && daughter('track').pt() >= 3.3) || (1. <= abs(daughter('track').eta()) && abs(daughter('track').eta()) < 2.2 && daughter('track').p() >= 2.9) || (2.2 <= abs(daughter('track').eta()) && daughter('track').pt() >= 0.8))")#&& daughter('track').pt() >= 0.8 && (daughter('track').p() >= 2.9 || abs(daughter('track').eta()) <1.)")
#
process.onia2MuMuPatGlbGlb.dimuonSelection       = cms.string("( ((daughter('muon1').charge + daughter('muon2').charge == 0) && 2.6 < mass && mass < 3.5) || ((daughter('muon1').charge + daughter('muon2').charge != 0) && 2.9 < mass && mass < 3.3)) && abs(daughter('muon1').innerTrack.dz - daughter('muon2').innerTrack.dz) < 3")
